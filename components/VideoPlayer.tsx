...
export default function VideoPlayer({ 
 ...
 <span className="mr-2"></span> Performance Video {videoUrl.includes('drive.google.com') ? '(GOOGLE DRIVE)' : `(${videoType?.toUpperCase()})`}</h3>
 <a
 href={videoUrl}
 ...
 src={embedUrl}
 title={title}
 className="absolute top-0 left-0 w-full h-full" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen
 loading="eager" />
 </div> ) : (
 <div className="space-y-3"> {/* Fallback when video can't be embedded */}
 <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
 <div className="flex items-start gap-3">
 <div className="flex-shrink-0 text-2xl"></div>
 <div className="flex-1">
 <p className="text-sm font-medium text-yellow-900 mb-2"> Video cannot be embedded directly
 </p>
 <p className="text-xs text-yellow-700 mb-3"> {videoUrl.includes('drive.google.com')
 ...
 <p className="text-xs font-mono text-gray-700 break-all"> {videoUrl}
 </p>
 </div>
 <a
 href={videoUrl}
 target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors" >
 <span className="mr-2"></span> Watch Video in New Tab
 </a>
 </div>
 </div>
 </div>
