...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
 ...
 const existingIndex = prev.findIndex(toast => toast.type === type && toast.message === message
 );
 ...
 return (
 <ToastContext.Provider value={{ showToast, success, error, warning, info, validationWarning, validationError }}> {children}
...
const ToastContainer: React.FC<{ toasts: Toast[]; onRemove: (id: string) => void }> = ({ toasts, onRemove }) => {
 ...
 return (
 <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 space-y-2 w-full max-w-sm px-4"> {toasts.map((toast) => (
