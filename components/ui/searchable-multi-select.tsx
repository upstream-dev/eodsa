...
export function SearchableMultiSelect({
 ...
 return (
 <div className={`relative ${className}`} ref={dropdownRef}> {/* Main Display */}
 <div
 className={`
 w-full border rounded-xl cursor-pointer transition-all min-h-[3rem]
 ${disabled 
 ? 'bg-gray-800 border-[rgba(192,192,192,0.15)] text-gray-500 cursor-not-allowed' 
 : isOpen
 ? 'border-[rgba(192,192,192,0.35)] bg-gray-700 text-white ring-2 ring-purple-500/20'
 : 'border-gray-600 bg-gray-700 text-white hover:border-purple-400'
 }
 `}
 onClick={() => !disabled && setIsOpen(!isOpen)}
 >
 <div className="p-3"> {/* Selected Items */}
 {selectedOptions.length > 0 ? (
 <div className="flex flex-wrap gap-2"> {selectedOptions.map((option) => (
 <div
 key={option.id}
 className="flex items-center space-x-2 bg-purple-600/80 text-white px-3 py-1 rounded-lg text-sm" onClick={(e) => e.stopPropagation()}
 >
 <span className="font-medium">{option.label}</span> {option.subtitle && (
 ...
 {showSelectedCount && (
 <div className="flex items-center text-gray-400 text-sm px-2"> {selectedOptions.length} selected
 ...
 <div className="flex items-center space-x-2"> {selectedOptions.length > 0 && (
 <button
 onClick={(e) => {
 e.stopPropagation();
 clearAll();
 ...
 {isOpen && !disabled && (
 <div className="absolute z-50 w-full mt-2 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl max-h-80 overflow-hidden"> {/* Search Input */}
 ...
 className="w-full px-3 py-2 pl-10 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-[rgba(192,192,192,0.45)] focus:border-[rgba(192,192,192,0.5)]" />
 <svg className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
 </svg> {isSearching && (
 <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
 <div className="w-4 h-4 border-2 border-[rgba(192,192,192,0.22)] border-t-purple-500 rounded-full animate-spin"></div>
 </div> )}
 </div>
 </div> {/* Options List */}
 <div className="max-h-48 overflow-y-auto"> {allOptions.length === 0 ? (
 ...
 key={option.id}
 className={`
 px-4 py-3 border-b border-[rgba(192,192,192,0.15)]/50 last:border-b-0 transition-colors
 ${isMaxReached 
 ...
 <div className={`font-medium ${isSelected ? 'text-[var(--chrome-light)]' : 'text-white'}`}> {option.label}
 </div> {option.subtitle && (
 ...
 <div className="px-4 py-2 border-t border-gray-600 bg-black/40">
 <div className="text-xs text-gray-400 flex justify-between">
 <span> Selected: {selectedIds.length}
 {maxSelections && ` / ${maxSelections}`}
 </span> {minSelections > 0 && selectedIds.length < minSelections && (
 <span className="text-yellow-400"> Need {minSelections - selectedIds.length} more
 </span> )}
 </div>
 </div> )}
 </div> )}
