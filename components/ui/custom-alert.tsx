...
export const AlertProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
 ...
 return (
 <AlertContext.Provider value={{ showAlert, showConfirm, showPrompt }}> {children}
 ...
 > {/* Title */}
 <h2 className="text-xl font-semibold text-gray-800 mb-4"> {alertState.type === 'confirm' ? 'Confirm Action' : 
 alertState.type === 'prompt' ? 'Input Required' : 
 alertState.variant === 'success' ? 'Success' :
 alertState.variant === 'error' ? 'Error' :
 alertState.variant === 'warning' ? 'Warning' : 'Information'}</h2> {/* Message */}
 <p className="text-gray-600 mb-6"> {alertState.message}
 </p> {/* Input field for prompt */}
 {alertState.type === 'prompt' && (
 <input
 type="text" className="w-full px-3 py-2 border border-gray-300 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 mb-6 text-gray-900 placeholder-gray-500" placeholder={alertState.placeholder || 'Enter value...'}
 ...
 {/* Buttons */}
 <div className="flex space-x-3"> {/* Cancel/Close Button */}
 {(alertState.type === 'confirm' || alertState.type === 'prompt') && (
 <button
 type="button" className="flex-1 px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md font-medium transition-colors" onClick={handleCancel}
 > Cancel
 </button> )}
 
 {/* Confirm/OK Button */}
 <button
 type="button" className={`flex-1 px-4 py-2 text-white rounded-md font-medium transition-colors ${
