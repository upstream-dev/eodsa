...
export function MultiSelectDancers({
 ...
 const filtered = results.filter(dancer => !selectedDancers.find(selected => selected.id === dancer.id)
 );
 ...
 return (
 <div className={`relative ${className}`} ref={dropdownRef}> {/* Selected Dancers Display */}
 ...
 <div className="flex flex-wrap gap-2"> {selectedDancers.map((dancer) => {
 ...
 }`}> {isEligible ? dancer.name.charAt(0) : ''}
 </div>
 <div>
 <div className="font-medium">{dancer.name}</div>
 <div className={`text-xs ${isEligible ? 'text-purple-200' : 'text-red-200'}`}> Age {dancer.age} • {dancer.studioName || 'Private'}
 ...
 </svg> {(isSearching || isInitialLoading) && (
 ...
 <span> {selectedDancers.length} selected
 {maxSelections && ` / ${maxSelections}`}
 </span> {minSelections > 0 && selectedDancers.length < minSelections && (
 <span className="text-yellow-400 font-medium animate-pulse"> Need {minSelections - selectedDancers.length} more
 </span> )}
 </div>
 </div>
 ...
 <div className="max-h-60 overflow-y-auto"> {/* Loading State */}
 ...
 <p className="text-sm"> {isInitialLoading ? 'Loading dancers...' : `Searching for "${searchQuery}"...`}
 </p>
 </div>
 </div> )}
 ...
 key={dancer.id}
 className={`
 px-4 py-3 border-b border-[rgba(192,192,192,0.15)]/50 last:border-b-0 transition-all hover:scale-[1.02]
 ${isMaxReached 
 ? 'text-gray-500 cursor-not-allowed opacity-50' 
 : isEligible
 ? 'cursor-pointer hover:bg-black/40 text-white'
 ...
 <div>
 <div className="font-medium">{dancer.name}</div>
 <div className={`text-sm ${isEligible ? 'text-gray-300' : 'text-red-400'}`}> {dancer.eodsaId && `${dancer.eodsaId} • `}
