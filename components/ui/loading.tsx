...
export const LoadingButton: React.FC<LoadingButtonProps> = ({
 ...
 success: 'bg-green-600 hover:bg-green-700 text-white border border-transparent focus:ring-[rgba(192,192,192,0.45)]',
...
export const LoadingCard: React.FC<LoadingCardProps> = ({ 
 ...
 <div>
 <h3 className="text-lg font-medium text-gray-900">{title}</h3> {message && (
...
export const LoadingOverlay: React.FC<LoadingOverlayProps> = ({ 
 ...
 <h3 className="mt-4 text-lg font-medium text-gray-900">{title}</h3> {message && (
...
export const TableSkeleton: React.FC<{ rows?: number; cols?: number }> = ({ 
 ...
 <div className="space-y-3"> {Array.from({ length: rows }, (_, rowIndex) => (
 <div key={rowIndex} className="grid gap-4" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}> {Array.from({ length: cols }, (_, colIndex) => (
