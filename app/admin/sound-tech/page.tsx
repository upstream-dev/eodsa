...
function SoundTechPage() {
 ...
 > Back to Admin</button> {liveEntries.length > 0 && (
 <button
 ...
 </div>  <div className="bg-white border border-gray-200 rounded-lg shadow p-6">
 <div className="flex items-center">
 ...
 placeholder="Search by item name, choreographer, or participant..." className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white text-black placeholder-gray-500 focus:ring-2 focus:ring-[rgba(192,192,192,0.45)] focus:border-[rgba(192,192,192,0.5)]" />
 </div>  <div>
 <label className="block text-sm font-medium text-black mb-2">Event</label>
 <select
 value={selectedEvent}
 onChange={(e) => setSelectedEvent(e.target.value)}
 className="px-3 py-2 rounded-lg border border-gray-300 bg-white text-black focus:ring-2 focus:ring-[rgba(192,192,192,0.45)] focus:border-[rgba(192,192,192,0.5)]" >
 ...
 <span className="mr-2"></span> Live Performances - Music Files ({liveEntries.length})</h2>
 </div> {liveEntries.length > 0 ? (
 <div className="divide-y divide-gray-200"> {liveEntries.map((entry) => (
 <div
 key={entry.id}
 className={`p-6 ${
 ...
 <span className="text-purple-600 font-semibold"> {entry.itemNumber || '#'}
 </span>
 </div>
 <div className="flex-1 min-w-0">
 <h3 className="text-lg font-semibold text-black truncate"> {entry.itemName}</h3>
 <p className="text-sm text-black"> {getPerformanceType(entry.participantIds)} • Style: {entry.itemStyle}
 ...
 </p> {/* Nicely formatted contestant/dancer names */}
 <div className="mb-2"> {entry.contestantName && entry.contestantName !== 'Unknown Contestant' ? (
 <div>
 <p className="text-lg font-bold text-black mb-1"> {entry.contestantName.includes(', ') ? (
 <span className="text-indigo-600">Group Performance</span> ) : (
 <span className="text-green-600">Solo Performance</span> )}
 </p>
 <div className="flex flex-wrap gap-2"> {entry.contestantName.split(', ').map((name, index) => (
 <span 
 key={index}
 ...
 <p className="text-lg font-bold text-red-500"> Unknown Contestant
 </p> )}
 </div> {selectedEvent === 'all' && (
 <p className="text-xs text-gray-600"> Event: {getEventName(entry.eventId)}
 </p> )}
 </div>
 </div> {entry.musicFileUrl && (
 <div className="mt-4">
 <MusicPlayer
 musicUrl={entry.musicFileUrl}
 filename={entry.musicFileName || entry.itemName}
 className="max-w-2xl" showDownload={true}
 ...
 {!entry.musicFileUrl && (
 <div className="mt-4">
 <div className="mb-2 p-3 border border-yellow-200 bg-yellow-50 text-yellow-800 rounded-md text-sm"> Upload outstanding — no track uploaded yet.
 </div>
 <MusicUpload
 currentFile={null}
 ...
 </div>  <div className="ml-4 flex flex-col items-end space-y-2">
 <span className={`px-2 py-1 text-xs font-medium rounded-full ${
 entry.approved
 ? 'bg-green-100 text-green-800'
 : 'bg-yellow-100 text-yellow-800'
 }`}> {entry.approved ? 'Approved' : 'Pending'}
 </span> {/* Completion Toggle - Local Only */}
 <button
 ...
 </button> {/* Remove Music Button */}
 <button
 onClick={() => removeMusic(entry.id, entry.itemName)}
 disabled={removingMusic.has(entry.id)}
 className="px-3 py-1 text-xs font-medium text-red-600 bg-red-50 border border-red-200 rounded-md hover:bg-red-100 hover:text-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" title="Remove music file - contestant will be able to re-upload" >
 ...
 </button> {/* Hide filename to avoid confusion */}
 </div>
 </div>
 </div> ))}
 </div> ) : (
 ...
 <span className="mr-2"></span> Virtual Performances - Video Links ({virtualEntries.length})</h2>
 </div> {virtualEntries.length > 0 ? (
 <div className="divide-y divide-gray-200"> {virtualEntries.map((entry) => (
 <div key={entry.id} className="p-6">
 <div className="flex items-start justify-between">
 <div className="flex-1 min-w-0">
 <div className="flex items-center space-x-3 mb-3">
 <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
 <span className="text-blue-600 font-semibold"> {entry.itemNumber || '#'}
 </span>
 </div>
 <div className="flex-1 min-w-0">
 <h3 className="text-lg font-semibold text-black truncate"> {entry.itemName}</h3>
 <p className="text-sm text-black"> by {entry.choreographer} • {getPerformanceType(entry.participantIds)}
 </p>
 <p className="text-xs text-gray-600"> {getEventName(entry.eventId)} • {entry.participantNames?.join(', ')}
 </p>
 </div>
 ...
 <div className="flex items-center justify-between">
 <div>
 <p className="text-sm font-medium text-blue-900"> {entry.videoExternalType?.toUpperCase()} Video
 </p>
 <p className="text-xs text-blue-700 truncate max-w-md"> {entry.videoExternalUrl}
 </p>
 </div>
 <a
 href={entry.videoExternalUrl}
 target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors" >
 Watch Video
 </a>
 </div>
 </div> )}
 ...
 </span>  <div className="text-right text-xs text-gray-600">
 <div>Mastery: {entry.mastery}</div>
 <div>Style: {entry.itemStyle}</div> {/* Duration hidden by request */}
 </div>
 </div>
 </div>
 </div> ))}
