...
function AdminDashboard() {
 ...
 <h2 className={`text-2xl font-bold ${themeClasses.accentGradientText}`}> Loading Avalon Admin Dashboard</h2>
 <p className={`${themeClasses.loadingText} font-medium animate-pulse`}>Preparing your dashboard...</p> {/* Progress Dots */}
 ...
 return (
 <div className={`min-h-screen avalon-shell ${themeClasses.mainBg}`}> {/* Enhanced Header - Mobile Optimized */}
 ...
 <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-3 sm:py-6 lg:py-8 gap-3 sm:gap-4">
 ...
 <p className={`${themeClasses.textSecondary} text-xs sm:text-sm font-medium truncate`}> Competition Management · <span className="brand-duo-text">Elements of Dance SA</span>
 </p>
 </div>
 </div>
 <div className="flex flex-wrap items-center gap-2 sm:gap-3"> {/* System Status */}
 ...
 <div className={`flex flex-wrap items-center gap-2 px-2 py-1 ${theme === 'dark' ? 'bg-[rgba(17,17,17,0.72)]' : 'bg-gray-100/50'} ${themeClasses.cardRadius} border ${themeClasses.cardBorder} backdrop-blur-sm`}>
 <Link 
 href="/admin/rankings" className={`avalon-tap inline-flex items-center justify-center gap-1.5 px-3 py-2 ${themeClasses.buttonPrimary} rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 hover:shadow-md`}
 >
 <span className="sm:hidden">Rankings</span>
 <span className="hidden sm:inline">Rankings</span>
 </Link>  <Link 
 href="/admin/scoring-approval" className={`avalon-tap inline-flex items-center justify-center gap-1.5 px-3 py-2 ${themeClasses.buttonSuccess} rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 hover:shadow-md`}
 >
 <span className="sm:hidden">Scores</span>
 <span className="hidden sm:inline">Score Approval</span>
 </Link>  <Link 
 href="/backend" className={`hidden md:inline-flex avalon-tap items-center gap-1.5 px-3 py-2 ${themeClasses.buttonSecondary} rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 hover:shadow-md`}
 >
 <span className="hidden sm:inline">Backend tools</span>
 </Link>  <div className={`hidden sm:block w-px h-6 ${theme === 'dark' ? 'bg-gray-600' : 'bg-gray-300'}`}></div>  <button
 onClick={handleLogout}
 className={`avalon-tap inline-flex items-center justify-center gap-1.5 px-3 py-2 ${themeClasses.buttonDanger} rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 hover:shadow-md`}
 >
 Logout
 </button>
 </div>
 ...
 </header>  <div className="avalon-container avalon-section"> {/* Global Database Clean Message */}
 ...
 <span className="text-lg sm:text-xl"> {cleanDatabaseMessage.includes('Error') || cleanDatabaseMessage.includes('') ? '' : ''}
 </span>
 ...
 { id: 'assignments', label: 'Assignments', icon: '', color: 'amber' },
 { id: 'dancers', label: 'Dancers', icon: '', color: 'rose' },
 { id: 'studios', label: 'Studios', icon: '', color: 'orange' },
 ...(isPhase2Enabled ? [{ id: 'music-tracking', label: 'Media', icon: '', color: 'cyan' }] : [])
 ].map((tab) => (
 tab.link ? (
 <Link
 key={tab.id}
 href={tab.link}
 className={`avalon-tab px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 text-sm sm:text-base ${themeClasses.textSecondary} ${theme === 'dark' ? 'hover:bg-black/40 hover:shadow-md' : 'hover:bg-white/80 hover:shadow-md'}`}
 >
 <span>{tab.label}</span>
 </Link> ) : (
 <button
 ...
 className={`avalon-tab px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 text-sm sm:text-base ${
 activeTab === tab.id
 ? 'btn-chrome !rounded-xl shadow-lg'
 : `${themeClasses.textSecondary} ${theme === 'dark' ? 'hover:bg-black/40 hover:shadow-md' : 'hover:bg-white/80 hover:shadow-md'}`
 }`}
 >
 <span>{tab.label}</span>
 </button> )
 ...
 <p className={`text-sm ${themeClasses.textSecondary} mb-4`}> This event will be removed from all active dashboards.
 <br /><br /> No data will be deleted.
 <br /><br /> Restore it later from Backend  Archived.
 <br /><br /> Type <span className="font-mono font-bold">ARCHIVE</span> to continue.
 </p>
 <p className={`text-sm font-semibold ${themeClasses.textPrimary} mb-2`}>{archiveConfirmEvent.name}</p>
 <input
 type="text" value={archiveConfirmText}
 onChange={(e) => setArchiveConfirmText(e.target.value)}
 placeholder="Type ARCHIVE" className={`w-full px-3 py-2 rounded-lg border ${themeClasses.cardBorder} ${themeClasses.cardBg} ${themeClasses.textPrimary} mb-4`}
 autoFocus
 />
 ...
 <div className="p-4 sm:p-6"> {loadingMusicTracking ? (
 ...
 </div> ) : musicTrackingData.length === 0 ? (
 ...
 <p className={`text-xl sm:text-2xl font-bold ${theme === 'dark' ? 'text-green-200' : 'text-green-700'}`}> {musicTrackingData.filter(entry => entry.musicFileUrl).length}
 </p>
 </div>
 </div>
 </div>
 ...
 <p className={`text-xl sm:text-2xl font-bold ${theme === 'dark' ? 'text-red-200' : 'text-red-700'}`}> {musicTrackingData.filter(entry => !entry.musicFileUrl && entry.entryType === 'live').length}
 </p>
 </div>
 </div>
 </div>
 ...
 <p className={`text-xl sm:text-2xl font-bold ${theme === 'dark' ? 'text-blue-200' : 'text-blue-700'}`}> {musicTrackingData.filter(entry => entry.entryType === 'virtual').length}
 </p>
 </div>
 </div>
 </div>
 ...
 <p className={`text-xl sm:text-2xl font-bold ${theme === 'dark' ? 'text-orange-200' : 'text-orange-700'}`}> {musicTrackingData.filter(entry => !entry.videoExternalUrl && entry.entryType === 'virtual').length}
 </p>
 </div>
 </div>
 </div>
 ...
 <div className="flex flex-col gap-3 sm:flex-row sm:gap-4"> {/* Search input */}
 <div className="flex-1">
 <input
 type="text" placeholder="Search by item name, contestant, or studio..." value={searchTerm}
 ...
 />
 </div> {/* Filter row for mobile */}
 <div className="flex gap-2 sm:gap-4 sm:flex-row"> {/* Entry Type Filter */}
 <select
 ...
 className="px-3 py-1.5 bg-red-600 text-white rounded-md text-xs font-semibold hover:bg-red-700" title="Clear music for all currently filtered live entries needing uploads" >
 Clear Music (Filtered)
 </button>
 <button
 onClick={bulkClearVideos}
 className="ml-2 px-3 py-1.5 bg-purple-600 text-white rounded-md text-xs font-semibold hover:bg-purple-700" title="Clear video links for all currently filtered virtual entries" >
 ...
 className="ml-2 px-2 py-1.5 border rounded-md text-xs" defaultValue="" title="Clear music/video links for selected event (does not delete Cloudinary files — use Purge Media on archived events)" >
 <option value="" disabled>Clear Event Media Links…</option> {events.map(ev => (
 <option key={ev.id} value={ev.id}>{ev.name}</option> ))}
 </select>
 </div> {/* Mobile swipe indicator */}
 ...
 <tr>
 <th className={`px-3 sm:px-6 py-3 text-left text-xs font-medium ${themeClasses.tableHeaderText} uppercase tracking-wider`}> Entry Details
 </th>
 <th className={`hidden sm:table-cell px-6 py-3 text-left text-xs font-medium ${themeClasses.tableHeaderText} uppercase tracking-wider`}> Contestant
 </th>
 <th className={`hidden lg:table-cell px-6 py-3 text-left text-xs font-medium ${themeClasses.tableHeaderText} uppercase tracking-wider`}> Event
 </th>
 <th className={`px-3 sm:px-6 py-3 text-left text-xs font-medium ${themeClasses.tableHeaderText} uppercase tracking-wider`}> Type
 </th>
 ...
 <th className={`px-3 sm:px-6 py-3 text-left text-xs font-medium ${themeClasses.tableHeaderText} uppercase tracking-wider`}> Actions
 </th>
 </tr>
 </thead>
 <tbody className={`${themeClasses.tableRow} divide-y ${themeClasses.tableBorder}`}> {musicTrackingData.filter((entry) => {
 // Search term filter
 ...
 <div className={`mb-2 p-2 border ${theme === 'dark' ? 'border-yellow-700 bg-yellow-900/20 text-yellow-200' : 'border-yellow-200 bg-yellow-50 text-yellow-800'} rounded-md text-xs`}> Upload outstanding — no track uploaded yet.
 </div>
 <MusicUpload
 currentFile={null}
 ...
 setMusicTrackingData(prev => prev.map(e => e.id === entry.id 
 ...
 <input
 type="url" placeholder={entry.videoExternalUrl ? 'Replace YouTube/Vimeo link…' : 'Paste YouTube/Vimeo link…'}
 value={videoLinkDrafts[entry.id] ?? ''}
 onChange={(e) => setVideoLinkDrafts(prev => ({ ...prev, [entry.id]: e.target.value }))}
 className={`flex-1 min-w-0 px-2 py-1.5 text-xs rounded border ${themeClasses.cardBorder} ${themeClasses.cardBg} ${themeClasses.textPrimary} focus:ring-[3px] focus:ring-[rgba(192,192,192,0.12)]`}
 />
 ...
 className="px-3 py-1.5 text-xs rounded bg-purple-600 text-white hover:bg-purple-700 transition-colors" >Save</button> {entry.videoExternalUrl && (
 <button
 onClick={async () => {
 try {
 const res = await fetch(`/api/admin/entries/${entry.id}`, {
 ...
 <td className="px-2 sm:px-6 py-3 sm:py-4"> {entry.entryType === 'live' ? (
 entry.musicFileUrl ? (
 ...
 <td className="px-2 sm:px-6 py-3 sm:py-4"> {entry.entryType === 'virtual' ? (
 ...
 <div className="flex flex-col sm:flex-row gap-1 sm:gap-2"> {entry.musicFileUrl && (
 <a
 href={entry.musicFileUrl}
 target="_blank" rel="noopener noreferrer" className={`${theme === 'dark' ? 'text-cyan-400 hover:text-[var(--chrome-mid)]' : 'text-cyan-600 hover:text-cyan-900'} transition-colors py-1 touch-manipulation`}
 >
 ...
 target="_blank" rel="noopener noreferrer" className={`${theme === 'dark' ? 'text-[var(--chrome-mid)] hover:text-[var(--chrome-light)]' : 'text-purple-600 hover:text-purple-900'} transition-colors py-1 touch-manipulation`}
 >
 ...
 <span className="sm:hidden"></span>
 </button> {/* Bulk action seed: individual clear button available above. A dedicated Bulk Clear panel can iterate IDs from current filter. */}
 </div>
 </td>
 </tr> ))}
 </tbody>
 ...
 className={`${themeClasses.textMuted} ${theme === 'dark' ? 'hover:text-white hover:bg-black/40' : 'hover:text-gray-900 hover:bg-gray-100/50'} p-2 rounded-lg transition-colors`}
 >
 <span className="text-2xl">×</span>
 </button>
 </div>
 </div>  <form onSubmit={handleCreateEvent} className={themeClasses.cardPadding}> {/* Event Type Selection */}
 ...
 <label className={`block ${themeClasses.label} mb-3 font-semibold`}> Event Type <span className="text-red-500">*</span>
 </label>
 ...
 ? theme === 'dark'
 ? 'bg-purple-600 border-[rgba(192,192,192,0.35)] text-white'
 ...
 <button
 type="button" onClick={() => setNewEvent(prev => ({ ...prev, eventMode: 'LIVE', participationMode: 'live' }))}
 className={`px-4 py-3 rounded-xl border-2 transition-all duration-200 font-medium ${
 ...
 <button
 type="button" onClick={() => setNewEvent(prev => ({ ...prev, eventMode: 'VIRTUAL', participationMode: 'virtual' }))}
 className={`px-4 py-3 rounded-xl border-2 transition-all duration-200 font-medium ${
 ...
 <button
 type="button" onClick={() => setNewEvent(prev => ({ ...prev, eventMode: 'HYBRID', participationMode: 'hybrid' }))}
 className={`px-4 py-3 rounded-xl border-2 transition-all duration-200 font-medium ${
 newEvent.eventMode === 'HYBRID'
 ? theme === 'dark'
 ? 'bg-[rgba(192,192,192,0.15)] border-indigo-500 text-white'
 ...
 <p className={`text-xs ${themeClasses.textMuted} mt-2`}> {newEvent.eventMode === 'LIVE' && 'Only live in-person entries will be allowed during registration.'}
 ...
 <input
 type="checkbox" checked={newEvent.qualificationRequired}
 ...
 </div> {newEvent.qualificationRequired && (
 ...
 <input
 type="number" min="0" max="100" value={newEvent.minimumQualificationScore || 75}
 onChange={(e) => setNewEvent(prev => ({ 
 ...prev, 
 ...
 className={`w-full px-4 py-3 ${themeClasses.inputBg} ${themeClasses.inputBorder} ${themeClasses.cardRadius} ${themeClasses.inputFocus} ${themeClasses.textPrimary}`}
 />
 <p className={`text-xs ${themeClasses.textMuted} mt-1`}> Dancers must score at least this percentage in a Regional Event to qualify
 </p>
 </div>
 </div> )}
 </div> )}
 ...
 </div> {(newEvent.qualificationSource === 'REGIONAL' || newEvent.qualificationSource === 'ANY_NATIONAL_LEVEL') && (
 <div>
 <label className={`block ${themeClasses.label} mb-2`}>Minimum Qualification Score (%) (Optional)</label>
 <input
 type="number" min="0" max="100" value={newEvent.minimumQualificationScore || ''}
 onChange={(e) => setNewEvent(prev => ({ 
 ...prev, 
 ...
 <h3 className={`${themeClasses.heading3} mb-4 flex items-center gap-2`}> Basic Information</h3>
 ...
 required
 />
 <p className={`text-xs ${themeClasses.textMuted} mt-1`}> Target number of judges for this event (1-10)
 </p>
 </div>
 </div>
 </div> {/* Dates Section */}
 <div className={`mb-6 p-6 border ${themeClasses.modalBorder} ${themeClasses.cardRadius} ${theme === 'dark' ? 'bg-[rgba(17,17,17,0.72)]' : 'bg-white'}`}>
 <h3 className={`${themeClasses.heading3} mb-4 flex items-center gap-2`}> Event Dates</h3>
 <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
 <div>
 ...
 <input
 type="datetime-local" value={newEvent.eventEndDate}
 ...
 <input
 type="datetime-local" value={newEvent.registrationDeadline}
 onChange={(e) => setNewEvent(prev => ({ ...prev, registrationDeadline: e.target.value }))}
 className={`w-full px-4 py-3 ${themeClasses.inputBg} ${themeClasses.inputBorder} ${themeClasses.cardRadius} ${themeClasses.inputFocus} ${themeClasses.textPrimary} transition-all duration-200`}
 required
 />
 <p className={`text-xs ${themeClasses.textMuted} mt-1`}> Registration closes at this date and time
 </p>
 </div>
 </div>
 ...
 <p className={`text-sm ${themeClasses.textPrimary}`}> If flat pricing values are set, legacy pricing is disregarded for new calculations.
 </p>
 </div>
 ...
 <p className={`text-sm font-semibold ${themeClasses.textPrimary}`}> Active pricing model: {getActivePricingModel(newEvent) === 'flat' ? 'Flat Pricing' : 'Legacy Pricing'}
 </p>
 <p className={`text-xs ${themeClasses.textMuted} mt-1`}> {getActivePricingModel(newEvent) === 'flat'
 ...
 <button
 type="button" onClick={() => setNewEvent(prev => activateFlatPricing(prev))}
 ...
 <button
 type="button" onClick={() => setNewEvent(prev => activateLegacyPricing(prev))}
 ...
 <input type="checkbox" checked={newEvent.discountEnabled} onChange={(e) => setNewEvent(prev => ({ ...prev, discountEnabled: e.target.checked }))} /> Discount enabled
 </label>
 <div>
 ...
 <p className={`text-xs ${themeClasses.textMuted} mb-2`}> Flat pricing: every Solo item uses the same Solo price (there is no separate &quot;Solo 4&quot; tier). For tiered solo pricing (e.g. different fee after the 3rd solo), use Legacy fields — especially <strong>Legacy fee after third solo</strong>.
 </p>
 ...
 const lineDisc = (soloIdx: number) => discEnabled && N > 0 && discAmt > 0 && soloIdx > 0 && soloIdx % N === 0
 ? Math.min(discAmt, sp)
 : 0;
 const grossA = sp * 3;
 ...
 <a
 href="/certificates/test" target="_blank" rel="noopener noreferrer" className={`text-sm px-4 py-2 ${theme === 'dark' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-purple-500 hover:bg-purple-600'} text-white rounded-lg transition-colors flex items-center gap-2`}
 >
 <span>🧪</span>
 ...
 <div className="flex-1">
 <p className={`text-sm font-medium ${theme === 'dark' ? 'text-blue-300' : 'text-blue-800'} mb-1`}> Recommended Template Dimensions
 </p>
 <p className={`text-xs ${theme === 'dark' ? 'text-blue-200' : 'text-blue-700'}`}> For best results, upload a template with dimensions: <strong>904px × 1280px</strong> (width × height)
 </p>
 <p className={`text-xs ${theme === 'dark' ? 'text-blue-200/80' : 'text-blue-600'} mt-1`}> Templates with different dimensions may cause text positioning issues. The system will warn you if dimensions don't match.
 </p>
 </div>
 </div>
 </div>  <div>
 <label className={`block ${themeClasses.label} mb-2`}>Upload Certificate Template (JPG/PNG)</label>
 <p className={`text-xs ${themeClasses.textMuted} mb-3`}> This will be used as the background for all certificates in this event. Only PNG and JPG files are accepted.
 </p> {certificateDimensionError && (
 ...
 <p className={`text-sm ${theme === 'dark' ? 'text-yellow-300' : 'text-yellow-800'}`}> {certificateDimensionError}
 </p>
 </div> )}
 ...
 <p className={`text-sm ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'} mb-2`}> 📄 Selected: {certificateTemplateFile.name}
 </p>
 <button
 type="button" onClick={() => {
 setCertificateTemplateFile(null);
 ...
 <input
 type="file" accept=".png,.jpg,.jpeg" onChange={async (e) => {
 ...
 <button
 type="button" onClick={() => setShowCreateEventModal(false)}
 className={`px-6 py-3 border ${theme === 'dark' ? 'border-gray-600 hover:bg-black/40' : 'border-gray-300 hover:bg-gray-50'} ${themeClasses.textSecondary} ${themeClasses.cardRadius} transition-colors font-medium`}
 > Cancel
 </button>
 <button
 type="submit" disabled={isCreatingEvent || isUploadingCertificate}
 ...
 </div>  <form onSubmit={handleUpdateEvent} className={themeClasses.cardPadding}>
 <div className={`mb-6 p-4 ${theme === 'dark' ? 'bg-blue-900/20 border-blue-700/50' : 'bg-blue-50 border-blue-200'} ${themeClasses.cardRadius} border`}>
 ...
 <h3 className={`text-sm font-semibold ${theme === 'dark' ? 'text-yellow-300' : 'text-yellow-800'} mb-2 flex items-center gap-2`}> Safety Warnings</h3>
 <ul className={`text-sm ${theme === 'dark' ? 'text-yellow-200' : 'text-yellow-700'} space-y-1 list-disc list-inside`}> {eventSafetyCheck.warnings.map((warning: string, idx: number) => (
 <li key={idx}>{warning}</li> ))}
 </ul> {eventSafetyCheck.stats && (
 ...
 <p className={`text-sm ${theme === 'dark' ? 'text-amber-100/90' : 'text-amber-950/90'}`}> You can edit pricing below. Existing payments are not automatically adjusted — confirm amounts against what families already paid before saving.
 </p>
 </div> )}
 ...
 <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
 <button
 type="button" onClick={() => setEditEventData(prev => ({ ...prev, participationMode: 'live' }))}
 className={`px-4 py-3 rounded-xl border-2 transition-all duration-200 font-medium ${
 ...
 <button
 type="button" onClick={() => setEditEventData(prev => ({ ...prev, participationMode: 'virtual' }))}
 className={`px-4 py-3 rounded-xl border-2 transition-all duration-200 font-medium ${
 ...
 <button
 type="button" onClick={() => setEditEventData(prev => ({ ...prev, participationMode: 'hybrid' }))}
 className={`px-4 py-3 rounded-xl border-2 transition-all duration-200 font-medium ${
 ...
 <p className={`text-xs ${themeClasses.textMuted} mt-2`}> {editEventData.participationMode === 'live' && 'Only live in-person entries will be allowed during registration.'}
 ...
 <input
 type="text" value={editEventData.name}
 ...
 <label className={`block ${themeClasses.label} mb-2`}> Number of Judges
 {eventSafetyCheck?.blocks.includes('judgeCount') && (
 <span className="ml-2 text-red-500 text-xs">( Cannot change - scores exist)</span> )}
 </label>
 <input
 type="number" min="1" max="10" value={editEventData.numberOfJudges}
 ...
 />
 <p className={`text-xs ${themeClasses.textMuted} mt-1`}> Current number of judges assigned to this event (for reference)
 {eventSafetyCheck?.stats?.scores > 0 && (
 <span className="block text-red-500 mt-1"> {eventSafetyCheck?.stats?.scores} score(s) exist - changing judge count could break calculations
 </span> )}
 </p>
 </div>  <div>
 <label className={`block ${themeClasses.label} mb-2`}>Event Date *</label>
 <input
 type="date" value={editEventData.eventDate}
 ...
 <input
 type="date" value={editEventData.eventEndDate}
 ...
 <input
 type="date" value={editEventData.registrationDeadline}
 ...
 <p className={`text-sm font-semibold ${themeClasses.textPrimary}`}> Active pricing model: {getActivePricingModel(editEventData) === 'flat' ? 'Flat Pricing' : 'Legacy Pricing'}
 </p>
 <p className={`text-xs ${themeClasses.textMuted} mt-1`}> {getActivePricingModel(editEventData) === 'flat'
 ? 'This event currently uses flat pricing for new calculations.'
 : 'This event currently uses legacy pricing for new calculations.'}
 </p>
 <div className="mt-3 flex flex-wrap gap-2">
 <button
 type="button" onClick={() => setEditEventData(prev => activateFlatPricing(prev))}
 ...
 <button
 type="button" onClick={() => setEditEventData(prev => activateLegacyPricing(prev))}
 ...
 <label className={`flex items-center gap-2 ${themeClasses.textPrimary}`}>
 <input type="checkbox" checked={(editEventData as any).discountEnabled} onChange={(e) => setEditEventData(prev => ({ ...prev, discountEnabled: e.target.checked }))} /> Discount enabled
 </label>
 <div>
 ...
 <label className={`block ${themeClasses.label} mb-2`}>Upload Certificate Template (JPG/PNG)</label>
 <p className={`text-xs ${themeClasses.textMuted} mb-3`}> This will be used as the background for all certificates in this event. Only PNG and JPG files are accepted.
 </p> {editCertificateDimensionError && (
 <div className={`mb-3 p-3 ${theme === 'dark' ? 'bg-yellow-900/30 border-yellow-700/50' : 'bg-yellow-50 border-yellow-200'} ${themeClasses.cardRadius} border`}>
 <p className={`text-sm ${theme === 'dark' ? 'text-yellow-300' : 'text-yellow-800'}`}> {editCertificateDimensionError}
 </p>
 </div> )}
 ...
 <p className={`text-sm font-medium ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'} mb-2`}> 📄 Current Template
 </p>
 <div className="mb-3">
 <img 
 src={editEventData.certificateTemplateUrl} 
 alt="Certificate Template Preview" className={`max-w-full h-auto max-h-48 rounded border ${themeClasses.modalBorder}`}
 onError={(e) => {
 ...
 target="_blank" rel="noopener noreferrer" className={`text-xs px-3 py-1 ${theme === 'dark' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white rounded transition-colors`}
 > View Full Size
 </a>
 <button
 ...
 <p className={`text-sm ${theme === 'dark' ? 'text-green-300' : 'text-green-700'} mb-2`}> 📄 New template selected: {editCertificateTemplateFile.name}
 </p>
 <button
 type="button" onClick={() => {
 setEditCertificateTemplateFile(null);
 ...
 <button
 type="submit" disabled={isUpdatingEvent || isUploadingEditCertificate}
 ...
 <div className="flex items-center justify-between">
 <div className="flex items-center space-x-3">
 <div className="w-10 h-10 bg-[rgba(192,192,192,0.1)] border border-[rgba(192,192,192,0.3)] rounded-xl flex items-center justify-center">
 <span className="text-white text-lg"></span>
 </div>
 ...
 </div>  <form onSubmit={handleTestEmailConnection} className="p-6">
 ...
 </div>  <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
 ...
 </div>  <div className="flex justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
 <button
 type="button" onClick={() => setShowEmailTestModal(false)}
 className={`px-6 py-3 border ${theme === 'dark' ? 'border-gray-600 hover:bg-black/40' : 'border-gray-300 hover:bg-gray-50'} ${themeClasses.textSecondary} rounded-xl transition-colors font-medium`}
 > Cancel
 </button>
 <button
 type="submit" disabled={isTestingEmail}
 className="inline-flex items-center space-x-3 px-8 py-3 btn-chrome !rounded-full text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 shadow-lg font-semibold" >
 {isTestingEmail ? (
 <>
 <div className="relative w-5 h-5">
 ...
 `}</style> {/* Financial Management Modal */}
 ...
 className={`${themeClasses.textMuted} ${theme === 'dark' ? 'hover:text-white hover:bg-black/40' : 'hover:text-gray-900 hover:bg-gray-100'} p-2 rounded-lg transition-colors`}
 >
 <span className="text-2xl">×</span>
 </button>
 </div>
 </div>  <div className="p-6 space-y-6"> {loadingFinances ? (
 <div className="text-center py-8">
 ...
 <span className="mr-2"></span> Registration Fees (per event)</h3> {selectedDancerFinances.financial?.registrationByEvent?.length > 0 ? (
 <div className="space-y-3"> {selectedDancerFinances.financial.registrationByEvent.map((reg: any) => (
 ...
 <div className={`text-xs ${themeClasses.textMuted}`}> {reg.charged ? `Paid${reg.chargedAt ? ` · ${new Date(reg.chargedAt).toLocaleDateString()}` : ''}` : 'Outstanding'}
 </div>
 </div>
 <span className={`text-sm font-bold ${reg.charged ? (theme === 'dark' ? 'text-[var(--chrome-mid)]' : 'text-green-600') : (theme === 'dark' ? 'text-red-400' : 'text-red-600')}`}> {reg.charged ? `R${reg.amount.toFixed(2)}` : `R${reg.outstanding.toFixed(2)} due`}
 </span>
 </div> ))}
 ...
 <span className={`text-sm font-bold ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}> R{selectedDancerFinances.financial?.registrationFeeOutstanding?.toFixed(2) || '0.00'}
 </span>
 </div>
 </div> ) : (
 <p className={`text-sm ${themeClasses.textSecondary}`}>No registration fees recorded yet.</p> )}
 </div> {/* Enhanced Balance Overview */}
 ...
 <h3 className={`${themeClasses.heading3} mb-3 flex items-center`}>
 <span className="mr-2"></span> Financial Summary</h3>
 ...
 <div className={`text-lg font-bold ${theme === 'dark' ? 'text-[var(--chrome-mid)]' : 'text-green-600'}`}> R{selectedDancerFinances.financial?.totalPaid?.toFixed(2) || '0.00'}
 </div>
 </div>
 <div className={`${themeClasses.cardBg} ${themeClasses.cardRadius} p-3 border ${themeClasses.cardBorder}`}>
 <div className={`text-xs font-medium ${themeClasses.textMuted} uppercase tracking-wider`}>Outstanding</div>
 <div className={`text-lg font-bold ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}> R{selectedDancerFinances.financial?.totalOutstanding?.toFixed(2) || '0.00'}
 </div>
 </div>
 </div>
 </div> {/* Solo Entries Section */}
 ...
 <h3 className={`${themeClasses.heading3} mb-3 flex items-center`}>
 <span className="mr-2">🕺</span> Solo Entries ({selectedDancerFinances.entries.soloCount})</h3>
 <div className="space-y-3"> {selectedDancerFinances.entries.solo.map((entry: any, index: number) => (
 ...
 <div className={`text-xs ${theme === 'dark' ? 'text-[var(--chrome-mid)]' : 'text-purple-600'} font-medium mt-1`}>Solo Performance</div>
 </div>
 ...
 <h3 className={`${themeClasses.heading3} mb-3 flex items-center`}>
 <span className="mr-2"></span> Group Entries ({selectedDancerFinances.entries.groupCount})</h3>
 <div className="space-y-4"> {selectedDancerFinances.entries.group.map((entry: any, index: number) => (
 ...
 </span> {entry.isMainContestant && (
 <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${theme === 'dark' ? 'bg-yellow-900/80 text-yellow-200' : 'bg-yellow-100 text-yellow-800'}`}> MAIN CONTESTANT
 </span> )}
 </div>
 </div>
 <div className="text-right ml-4">
 <div className={`text-xs ${themeClasses.textMuted} mb-1`}> {entry.isMainContestant ? 'Full Fee' : 'Your Share'}
 </div>
 <div className={`text-sm font-bold ${themeClasses.textPrimary}`}> R{entry.dancerShare?.toFixed(2) || '0.00'}
 </div> {!entry.isMainContestant && (
 <div className={`text-xs ${themeClasses.textMuted}`}> of R{entry.calculatedFee?.toFixed(2) || '0.00'}
 </div> )}
 ...
 <div className="flex flex-wrap gap-1"> {entry.participantNames?.map((name: string, nameIndex: number) => (
 ...
 <h3 className={`${themeClasses.heading3} mb-3 flex items-center`}>
 <span className="mr-2"></span> Outstanding Balance Breakdown</h3>
 <div className="space-y-3"> {/* Registration Fee */}
 <div className="flex justify-between items-center">
 <span className={`text-sm ${themeClasses.textSecondary}`}>Registration Fee:</span>
 <span className={`text-sm font-medium ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}> R{selectedDancerFinances.financial?.registrationFeeOutstanding?.toFixed(2) || '0.00'}
 </span>
 </div> {/* Entry Fees Breakdown */}
 <div className="space-y-2">
 <div className="flex justify-between items-center">
 <span className={`text-sm ${themeClasses.textSecondary}`}>Solo Entries Outstanding:</span>
 <span className={`text-sm font-medium ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}> R{(selectedDancerFinances.entries?.solo?.filter((e: any) => e.paymentStatus !== 'paid')
 ...
 <span className={`text-sm font-medium ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}> R{(selectedDancerFinances.entries?.group?.filter((e: any) => e.paymentStatus !== 'paid')
 ...
 <span className={`text-xl font-bold ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}> R{selectedDancerFinances.financial?.totalOutstanding?.toFixed(2) || '0.00'}
 </span>
 ...
 <span className={`text-xs ${themeClasses.textMuted}`}> {selectedDancerFinances.entries.all?.filter((e: any) => e.paymentStatus === 'paid').length || 0} of {selectedDancerFinances.entries.totalEntries} entries paid
 </span>
 </div>
 ...
 <div className={`${theme === 'dark' ? 'bg-blue-900/20' : 'bg-blue-50'} ${themeClasses.cardRadius} p-4 border ${theme === 'dark' ? 'border-blue-700/50' : 'border-blue-200'}`}>
 <h3 className={`${themeClasses.heading3} mb-3 flex items-center`}>
 <span className="mr-2">⚡</span> Quick Actions</h3>
 <div className="space-y-2">
 <button
 onClick={() => {
 ...
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> First Name *
 </label>
 <input
 type="text" value={editDancerData.name}
 onChange={(e) => setEditDancerData({ ...editDancerData, name: e.target.value })}
 className={`w-full px-3 py-2 ${themeClasses.cardBg} border ${themeClasses.cardBorder} rounded-lg ${themeClasses.textPrimary} focus:ring-[3px] focus:ring-[rgba(192,192,192,0.12)] focus:border-transparent`}
 placeholder="First name" />
 </div> {/* Surname */}
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> Surname *
 </label>
 <input
 type="text" value={editDancerData.surname}
 onChange={(e) => setEditDancerData({ ...editDancerData, surname: e.target.value })}
 className={`w-full px-3 py-2 ${themeClasses.cardBg} border ${themeClasses.cardBorder} rounded-lg ${themeClasses.textPrimary} focus:ring-[3px] focus:ring-[rgba(192,192,192,0.12)] focus:border-transparent`}
 placeholder="Surname" />
 </div> {/* National ID */}
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> National ID *
 </label>
 <input
 ...
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> Date of Birth
 </label>
 <input
 ...
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> Email
 </label>
 <input
 ...
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> Phone
 </label>
 <input
 ...
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> Guardian Name
 </label>
 <input
 type="text" value={editDancerData.guardianName}
 onChange={(e) => setEditDancerData({ ...editDancerData, guardianName: e.target.value })}
 className={`w-full px-3 py-2 ${themeClasses.cardBg} border ${themeClasses.cardBorder} rounded-lg ${themeClasses.textPrimary} focus:ring-[3px] focus:ring-[rgba(192,192,192,0.12)] focus:border-transparent`}
 placeholder="Guardian name" />
 </div> {/* Guardian Email */}
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> Guardian Email
 </label>
 <input
 type="email" value={editDancerData.guardianEmail}
 onChange={(e) => setEditDancerData({ ...editDancerData, guardianEmail: e.target.value })}
 className={`w-full px-3 py-2 ${themeClasses.cardBg} border ${themeClasses.cardBorder} rounded-lg ${themeClasses.textPrimary} focus:ring-[3px] focus:ring-[rgba(192,192,192,0.12)] focus:border-transparent`}
 placeholder="Guardian email" />
 </div> {/* Guardian Phone */}
 <div>
 <label className={`block text-sm font-medium ${themeClasses.textSecondary} mb-1`}> Guardian Phone
 </label>
 <input
 type="tel" value={editDancerData.guardianPhone}
 onChange={(e) => setEditDancerData({ ...editDancerData, guardianPhone: e.target.value })}
 className={`w-full px-3 py-2 ${themeClasses.cardBg} border ${themeClasses.cardBorder} rounded-lg ${themeClasses.textPrimary} focus:ring-[3px] focus:ring-[rgba(192,192,192,0.12)] focus:border-transparent`}
 placeholder="Guardian phone" />
 </div>
 </div>  <div className={`flex justify-end space-x-3 pt-4 border-t ${themeClasses.cardBorder}`}>
 <button
 onClick={() => {
 setShowEditDancerModal(false);
 setEditingDancer(null);
 ...
 <div>
 <h2 className={`text-xl font-bold ${themeClasses.textPrimary}`}>Studio Dashboard</h2> {selectedStudioProfile && (
 ...
 <div className={`text-xs mt-2 ${themeClasses.textMuted}`}> Approved: {new Date(studio.approvedAt).toLocaleDateString()}
 </div> )}
 </div>
 </div>
 ...
 <div className={`text-2xl font-bold text-[var(--chrome-mid)]`}>R{((financial.totalPaid ?? 0)).toLocaleString()}</div>
 </div>
 <div className={`rounded-xl border ${themeClasses.cardBorder} p-4 ${themeClasses.cardBg}`}>
 ...
 <tr>
 <th className={`px-6 py-3 text-left text-xs font-semibold ${themeClasses.textMuted} uppercase tracking-wider`}> Name
 </th>
 <th className={`px-6 py-3 text-left text-xs font-semibold ${themeClasses.textMuted} uppercase tracking-wider`}> EODSA ID
 </th>
 <th className={`px-6 py-3 text-left text-xs font-semibold ${themeClasses.textMuted} uppercase tracking-wider`}> Age
 </th>
 <th className={`px-6 py-3 text-left text-xs font-semibold ${themeClasses.textMuted} uppercase tracking-wider`}> Mastery Level
 </th>
 <th className={`px-6 py-3 text-left text-xs font-semibold ${themeClasses.textMuted} uppercase tracking-wider`}> Actions
 </th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-800"> {!Array.isArray(dancers) || dancers.length === 0 ? (
 <tr>
 <td colSpan={5} className={`px-6 py-6 text-center ${themeClasses.textMuted} text-sm`}> No registered dancers found.
 </td>
 </tr> ) : (
 ...
 <td className="px-6 py-3 text-sm"> {dancer?.masteryLevel && typeof dancer.masteryLevel === 'string' ? (
 ...
 <td className="px-6 py-3 text-sm"> {dancer?.eodsaId ? (
 <Link
 href={`/admin/dancers/${dancer.eodsaId}`}
 className="text-blue-400 hover:text-blue-300 underline text-xs" onClick={() => setShowStudioModal(false)}
 > View Profile 
 </Link> ) : (
 <span className={themeClasses.textMuted}>—</span> )}
 </td>
...
function JudgeAssignmentsTabContent({
 ...
 </div>  <div className="p-6"> {loadingAssignments ? (
 ...
 <span className={`px-2 py-1 bg-amber-500/20 text-amber-300 rounded text-xs`}> {event.status}
 </span>
 </div>
 <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
 <div>
 <span className={themeClasses.textMuted}>Date: </span>
 <span className={themeClasses.textPrimary}> {new Date(event.eventDate).toLocaleDateString()}
 </span>
 </div>
 <div>
 ...
 <span className={themeClasses.textPrimary}> {event.judges?.length || 0}/{event.numberOfJudges || event.expectedJudges || 4}
 {((event.judges?.length || 0) < (event.numberOfJudges || event.expectedJudges || 4)) && (
 <span className="text-amber-500 ml-1" title={`Event needs ${event.numberOfJudges || event.expectedJudges || 4} judges`}>  </span> )}
 </span>
 </div>
 </div>
 </div>
 ...
 <div className="flex items-center justify-between mb-4">
 <h4 className={`text-lg font-semibold ${themeClasses.textPrimary}`}> Assigned Judges ({event.judges?.length || 0}/{event.numberOfJudges || event.expectedJudges || 4})</h4> {(event.judges?.length || 0) >= (event.numberOfJudges || event.expectedJudges || 4) ? (
 <div className="px-4 py-2 bg-gray-500 text-white rounded-lg text-sm cursor-not-allowed opacity-50"> Maximum Reached ({event.numberOfJudges || event.expectedJudges || 4}/{event.numberOfJudges || event.expectedJudges || 4})
 </div> ) : (
 <button
 onClick={() => openAddJudgeModal(event.id)}
 className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed" disabled={getAvailableJudgesForEvent(event.id).length === 0}
 > + Add Judge
 </button> )}
 </div> {!event.judges || event.judges.length === 0 ? (
 <div className={`text-center py-8 ${themeClasses.textMuted}`}>
 <p>No judges assigned to this event yet.</p> {(event.judges?.length || 0) < (event.numberOfJudges || event.expectedJudges || 4) && getAvailableJudgesForEvent(event.id).length > 0 && (
 <button
 onClick={() => openAddJudgeModal(event.id)}
 ...
 <div
 key={judge.id}
 className={`${theme === 'dark' ? 'bg-black/40' : 'bg-gray-100'} rounded-lg p-4 flex items-center justify-between`}
 >
 <div className="flex items-center space-x-4">
 <div className="w-10 h-10 bg-amber-600 rounded-full flex items-center justify-center text-white font-bold"> {index + 1}
 </div>
 <div>
 <div className={themeClasses.textPrimary + ' font-medium'}>{judge.name}</div>
 <div className={themeClasses.textMuted + ' text-sm'}>{judge.email}</div> {judge.phone && (
 ...
 <label className={`block text-sm font-medium ${themeClasses.textPrimary} mb-2`}> Select Judge
 ...
 : theme === 'dark' 
 ? 'bg-black/40 hover:bg-gray-700 border-2 border-transparent' 
 ...
 </div>  <div className="p-6"> {staffAccounts.length === 0 ? (
 <div className={`text-center py-12 ${themeClasses.textMuted}`}>
 <div className="w-16 h-16 mx-auto mb-4 bg-gray-100 rounded-full flex items-center justify-center">
 <span className="text-2xl"></span>
 </div>
 ...
 <div className="space-y-4"> {staffAccounts.map((client) => (
 <div
 key={client.id}
 className={`${themeClasses.cardBg} rounded-xl border ${themeClasses.cardBorder} p-4 sm:p-6`}
 >
 ...
 <span className={`px-2 py-1 ${client.isApproved ? 'bg-green-500/20 text-green-300' : 'bg-yellow-500/20 text-yellow-300'} rounded text-xs`}> {client.isApproved ? 'Approved' : 'Pending'}
 </span>
 </div>
 ...
 <span className={themeClasses.textPrimary}> {client.allowedDashboards?.length || 0} enabled
 </span>
 </div>
 </div>
 </div>
 ...
 </div> {expandedStaffId === client.id && (
 <div className="mt-4 pt-4 border-t border-[rgba(192,192,192,0.15)]/20">
 <h4 className={`text-lg font-semibold ${themeClasses.textPrimary} mb-4`}> Dashboard Permissions</h4>
 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3"> {AVAILABLE_DASHBOARDS.map((dashboard) => {
 ...
 : 'bg-gray-700/30 border-[rgba(192,192,192,0.15)]/50'
...
function EventsTabContent({
 ...
 <div className="space-y-6 sm:space-y-8 animate-fadeIn">
 <div className={`${themeClasses.cardBg} backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border ${themeClasses.cardBorder}`}>
 <div className={`px-4 sm:px-6 py-3 sm:py-4 bg-[rgba(192,192,192,0.06)] border-b ${themeClasses.cardBorder}`}>
 <div className="flex items-center justify-between">
 <div className="flex items-center space-x-3">
 <div className="w-6 h-6 sm:w-8 sm:h-8 bg-[rgba(192,192,192,0.1)] border border-[rgba(192,192,192,0.3)] rounded-lg flex items-center justify-center flex-shrink-0">
 <span className="text-white text-xs sm:text-sm"></span>
 </div>
 <h2 className={`text-lg sm:text-xl font-bold ${themeClasses.textPrimary}`}>Events</h2>
 <div className={`px-2 sm:px-3 py-1 ${theme === 'dark' ? 'bg-indigo-900/80 text-indigo-200' : 'bg-indigo-100 text-indigo-800'} rounded-full text-xs sm:text-sm font-medium`}> {events.length}
 </div>
 </div>
 <button
 onClick={() => {
 ...
 isPhase2Enabled
 ? 'btn-chrome !rounded-full text-white transform hover:scale-105 shadow-lg'
 : 'bg-gray-500/50 text-gray-400 cursor-not-allowed opacity-50'
 }`}
 ...
 isPhase2Enabled
 ? 'bg-indigo-500 text-white hover:bg-[rgba(192,192,192,0.15)]'
 : 'bg-gray-500/50 text-gray-400 cursor-not-allowed opacity-50'
 }`}
 title={!isPhase2Enabled ? 'This feature is temporarily unavailable.' : ''}
 >
 ...
 <tbody className={`${themeClasses.tableRow} divide-y ${themeClasses.tableBorder}`}> {events.map((event) => (
 ...
 <div className="space-y-1"> {/* Show Event Mode (LIVE, VIRTUAL, HYBRID) */}
 {(() => {
 ...
 <div className="hidden sm:block"> {new Date(event.eventDate).toLocaleDateString()}
 </div>
 <div className="sm:hidden"> {new Date(event.eventDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
 </div>
 </td>
 <td className="px-3 sm:px-6 py-3 sm:py-4">
 <span className={`inline-flex px-2 sm:px-3 py-1 text-xs font-bold rounded-full border ${
 ...
 <span className="sm:hidden"> {event.status === 'upcoming' ? 'UPCOMING' : 
...
function StaffTabContent({ judges, setShowCreateJudgeModal, handleDeleteJudge, theme, themeClasses }: StaffTabContentProps) {
 ...
 <div className="space-y-8 animate-fadeIn">
 <div className={`${themeClasses.cardBg} backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border ${themeClasses.cardBorder}`}>
 <div className={`px-6 py-4 bg-[rgba(255,45,161,0.06)] border-b ${themeClasses.cardBorder}`}>
 <div className="flex items-center justify-between">
 <div className="flex items-center space-x-3">
 <div className="w-8 h-8 bg-[rgba(192,192,192,0.1)] border border-[rgba(192,192,192,0.3)] rounded-lg flex items-center justify-center">
 <span className="text-white text-sm"></span>
 </div>
 <h2 className={`text-xl font-bold ${themeClasses.textPrimary}`}>Staff Management</h2>
 <div className={`px-3 py-1 ${theme === 'dark' ? 'bg-purple-900/80 text-purple-200' : 'bg-purple-100 text-purple-800'} rounded-full text-sm font-medium`}> {judges.length} staff members
 </div>
 </div>
 <button
 onClick={() => setShowCreateJudgeModal(true)}
 className="inline-flex items-center space-x-2 px-4 py-2.5 btn-chrome !rounded-full text-white rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg font-medium" >
 <span></span>
 ...
 <tbody className={`${themeClasses.tableRow} divide-y ${themeClasses.tableBorder}`}> {judges.map((judge) => (
 ...
 <td className="px-6 py-4"> {judge.isAdmin ? (
 <span className="inline-flex px-3 py-1 text-xs font-bold rounded-full border btn-chrome !rounded-full text-white border-purple-300"> Admin
 </span> ) : (
 <div className="flex flex-wrap gap-1">
 <span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 border border-blue-200"> 👨‍ Judge
 </span>
 </div> )}
 </td>
 <td className={`px-6 py-4 text-sm font-medium ${themeClasses.textSecondary} hidden md:table-cell`}> {new Date(judge.createdAt).toLocaleDateString()}
 </td>
 <td className="px-6 py-4">
 <div className="flex space-x-2"> {!judge.isAdmin && (
 <>
 <button
 onClick={() => {}}
...
function AssignmentsTabContent({ assignments, setShowAssignJudgeModal, handleUnassignJudge, unassigningJudges, theme, themeClasses }: AssignmentsTabContentProps) {
 ...
 <div className={`px-3 py-1 ${theme === 'dark' ? 'bg-pink-900/80 text-pink-200' : 'bg-pink-100 text-pink-800'} rounded-full text-sm font-medium`}> {assignments.length} assignments
 </div>
 </div>
 <button
 ...
 <tbody className={`${themeClasses.tableRow} divide-y ${themeClasses.tableBorder}`}> {assignments.map((assignment) => (
 ...
 <td className="px-6 py-4 text-sm font-medium"> {assignment.eventName}
 ...
 className="inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-red-500 to-rose-600 text-white text-xs font-medium rounded-lg hover:from-red-600 hover:to-rose-700 transition-all duration-200 transform hover:scale-105 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed" title="Remove this judge from the event" >
 <span className="mr-1">{unassigningJudges.has(assignment.id) ? '⏳' : ''}</span>
...
function DancersTabContent({ dancers, dancerSearchTerm, setDancerSearchTerm, dancerStatusFilter, setDancerStatusFilter, handleApproveDancer, handleRejectDancer, handleViewFinances, handleRegistrationFeeUpdate, handleEditDancer, theme, themeClasses }: DancersTabContentProps) {
 ...
 <div className={`px-3 py-1 ${theme === 'dark' ? 'bg-rose-900/80 text-rose-200' : 'bg-rose-100 text-rose-800'} rounded-full text-sm font-medium`}> {filteredDancers.length} of {dancers.length} dancers
 </div>
 </div>  <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
 <div className="relative">
 <input
 type="text" placeholder="Search dancers..." value={dancerSearchTerm}
 ...
 </div> {filteredDancers.length === 0 ? (
 <div className={`text-center py-12 ${themeClasses.textMuted}`}>
 <div className="w-16 h-16 mx-auto mb-4 bg-gray-100 rounded-full flex items-center justify-center">
 <span className="text-2xl"></span>
 </div>
 <h3 className="text-lg font-medium mb-2"> {dancers.length === 0 ? 'No dancer registrations yet' : 'No dancers match your filters'}</h3>
 <p className="text-sm mb-4"> {dancers.length === 0 
 ...
 <tbody className={`${themeClasses.tableRow} divide-y ${themeClasses.tableBorder}`}> {filteredDancers.map((dancer) => (
 ...
 <div className={`text-xs ${themeClasses.textMuted}`}> Competition age{dancer.nationalsReferenceDate ? ` (as of ${dancer.nationalsReferenceDate})` : ''}
 </div>
 ...
 <td className="px-6 py-4"> {dancer.studioName ? (
 <div>
 ...
 <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-blue-900/80 text-blue-200' : 'bg-blue-100 text-blue-800'} mt-1`}> Studio Dancer
 </div>
 </div> ) : (
 <div>
 <div className={`text-sm font-medium ${theme === 'dark' ? 'text-[var(--chrome-mid)]' : 'text-purple-600'}`}>🕺 Independent</div>
 <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-purple-900/80 text-purple-200' : 'bg-purple-100 text-purple-800'} mt-1`}> Individual
 </div>
 </div> )}
 </td>
 <td className="px-6 py-4 hidden sm:table-cell"> {dancer.guardianName ? (
 <div>
 ...
 <td className="px-6 py-4"> {dancer.approved ? (
 <div>
 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-green-900/80 text-green-200' : 'bg-green-100 text-green-800'}`}> Approved
 </span> {dancer.approvedAt && (
 <div className={`text-xs ${themeClasses.textMuted} mt-1`}> {new Date(dancer.approvedAt).toLocaleDateString()}
 </div> )}
 </div> ) : dancer.rejectionReason ? (
 <div>
 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-red-900/80 text-red-200' : 'bg-red-100 text-red-800'}`}> Rejected
 </span>
 <div className={`text-xs ${themeClasses.textMuted} mt-1`} title={dancer.rejectionReason}> {dancer.rejectionReason.length > 30 
 ? dancer.rejectionReason.substring(0, 30) + '...' 
 : dancer.rejectionReason}
 </div>
 </div> ) : (
 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-yellow-900/80 text-yellow-200' : 'bg-yellow-100 text-yellow-800'}`}> ⏳ Pending
 </span> )}
 </td>
 <td className="px-6 py-4">
 <div className="space-y-2"> {!dancer.approved && !dancer.rejectionReason ? (
 <div className="flex space-x-2">
 <button
 onClick={() => handleApproveDancer(dancer.id)}
 className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[rgba(192,192,192,0.45)] transition-colors" >
  Approve
 </button>
 <button
 onClick={() => handleRejectDancer(dancer.id)}
 className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors" >
 <span className="text-white"></span> Reject
 </button>
 </div> ) : (
...
function StudiosTabContent({ studios, studioSearchTerm, setStudioSearchTerm, studioStatusFilter, setStudioStatusFilter, handleViewStudio, handleApproveStudio, handleRejectStudio, theme, themeClasses }: StudiosTabContentProps) {
 ...
 <div className={`px-3 py-1 ${theme === 'dark' ? 'bg-orange-900/80 text-orange-200' : 'bg-orange-100 text-orange-800'} rounded-full text-sm font-medium`}> {filteredStudios.length} of {studios.length} studios
 </div>
 </div>  <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
 <div className="relative">
 <input
 type="text" placeholder="Search studios..." value={studioSearchTerm}
 ...
 </div> {filteredStudios.length === 0 ? (
 <div className={`text-center py-12 ${themeClasses.textMuted}`}>
 <div className="w-16 h-16 mx-auto mb-4 bg-gray-100 rounded-full flex items-center justify-center">
 <span className="text-2xl"></span>
 </div>
 <h3 className="text-lg font-medium mb-2"> {studios.length === 0 ? 'No studio registrations yet' : 'No studios match your filters'}</h3>
 <p className="text-sm mb-4"> {studios.length === 0 
 ...
 <tbody className={`${themeClasses.tableRow} divide-y ${themeClasses.tableBorder}`}> {filteredStudios.map((studio) => (
 ...
 <td className="px-6 py-4"> {studio.approved ? (
 <div>
 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-green-900/80 text-green-200' : 'bg-green-100 text-green-800'}`}> Approved
 </span> {studio.approvedAt && (
 <div className={`text-xs ${themeClasses.textMuted} mt-1`}> {new Date(studio.approvedAt).toLocaleDateString()}
 </div> )}
 </div> ) : studio.rejectionReason ? (
 <div>
 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-red-900/80 text-red-200' : 'bg-red-100 text-red-800'}`}> Rejected
 </span>
 <div className={`text-xs ${themeClasses.textMuted} mt-1`} title={studio.rejectionReason}> {studio.rejectionReason.length > 30 
 ...
 className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors" >
  View Studio
 </button> {!studio.approved && !studio.rejectionReason && (
 <div className="flex space-x-2">
 <button
