...
function AdminCertificatesPageContent() {
 ...
 {error && (
 <div className={`${themeClasses.badgeRed} px-4 py-3 ${themeClasses.cardRadius} mb-6`}> {error}
 </div> )}
 {success && (
 <div className={`${themeClasses.badgeGreen} px-4 py-3 ${themeClasses.cardRadius} mb-6`}> {success}
 </div> )}
 ...
 <option value="">All Ages</option> {Array.from(new Set(rankings.map(r => r.ageCategory))).sort().map(age => (
 ...
 className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" >
 <option value="">All Types</option> {Array.from(new Set(rankings.map(r => r.performanceType))).sort().map(type => (
 ...
 className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" >
 <option value="">All Styles</option> {Array.from(new Set(rankings.map(r => r.itemStyle))).sort().map(style => (
 ...
 <div className="mt-3 text-sm text-gray-600"> Showing {filteredRankings.length} of {rankings.length} rankings
 </div>
 </div> {/* Quick Select */}
 <div className="bg-white rounded-lg shadow-md p-4 mb-6">
 ...
 <span className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-semibold"> Selected: {selectedWinners.size}
 </span> {selectedWinners.size > 0 && (
 <button
 onClick={() => {
 ...
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Rank
 </th>
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Dancer
 </th>
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Studio Name
 </th>
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Title
 </th>
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Style
 </th>
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Score
 </th>
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Medal
 </th>
 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Actions
 </th>
 </tr>
 </thead>
 <tbody className="bg-white divide-y divide-gray-200"> {filteredRankings.map((ranking, index) => (
 <tr key={ranking.performanceId} className={selectedWinners.has(ranking.performanceId) ? 'bg-blue-50' : ''}>
 <td className="px-6 py-4 whitespace-nowrap">
 <input
 type="checkbox" checked={selectedWinners.has(ranking.performanceId)}
 ...
 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900"> {ranking.studioName || (ranking.performanceType === 'Solo' ? 'Independent' : 'N/A')}
 </td>
 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900"> {ranking.title}
 </td>
 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900"> {ranking.itemStyle}
 </td>
 <td className="px-6 py-4 whitespace-nowrap">
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> {filteredCertificates.map((cert) => (
 ...
 <p>Medal: {cert.medallion}</p> {cert.eventName && (
 ...
 target="_blank" rel="noopener noreferrer" className="flex-1 px-4 py-2 bg-blue-100 text-blue-700 text-center rounded-lg hover:bg-blue-200 transition-colors text-sm" >
 📥 Download
 </a>
 <button
 ...
 {showConfirmGenerate && (
 <div
 className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setShowConfirmGenerate(false)}
 >
 <div className="relative bg-white rounded-xl p-8 max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
 <h3 className="text-2xl font-bold text-gray-900 mb-4"> Confirm Certificate Generation</h3>
 <p className="text-gray-600 mb-6"> You are about to generate <span className="font-bold text-blue-600">{selectedWinners.size} certificate(s)</span>.
 </p>
 <div className="bg-gray-50 rounded-lg p-4 mb-6">
 <p className="text-sm text-gray-700 mb-2">
 <strong>Preview:</strong> Click " Preview First" button to see how certificates will look
 </p>
 ...
 {previewUrl && (
 <div
 className="fixed inset-0 bg-black bg-opacity-90 z-50 overflow-auto" onClick={() => { setPreviewUrl(null); setPreviewRanking(null); }}
 >
 <div className="max-w-7xl mx-auto p-6" onClick={(e) => e.stopPropagation()}>
 <div className="flex justify-between items-center mb-4">
 <h2 className="text-2xl font-bold text-white"> {previewRanking?.contestantName} - Adjust Certificate</h2>
 ...
 </div>  <div className="grid grid-cols-1 lg:grid-cols-4 gap-6"> {/* Controls */}
 <div className="bg-white rounded-lg p-4 max-h-[85vh] overflow-y-auto">
 <h3 className="font-bold text-gray-900 mb-4">Position Adjustments</h3> {/* Name */}
 ...
 key={previewUrl}
 src={previewUrl}
 alt="Certificate Preview" className="max-w-full rounded-lg shadow-2xl" />
 </div>
 </div>
 </div>
 </div> )}
