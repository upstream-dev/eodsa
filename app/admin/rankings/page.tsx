...
function AdminRankingsPage() {
 ...
 filtered = Object.values(groupedByAge).flatMap(group => group.sort((a, b) => b.totalScore - a.totalScore).slice(0, 3)
 );
 ...
 filtered = Object.values(groupedByStyle).flatMap(group => group.sort((a, b) => b.totalScore - a.totalScore).slice(0, 3)
 );
 ...
 setRankings((current) => current.map((ranking) => ranking.performanceId === performanceId ? { ...ranking, announced } : ranking
 )
 );
 ...
 setRankings((current) => current.map((ranking) => targetIds.has(ranking.performanceId) ? { ...ranking, announced: true } : ranking
 )
 );

 try {
 const updates = targets.map((ranking) => fetch(`/api/performances/${ranking.performanceId}/toggle-announced`, {
 method: 'PATCH',
 headers: { 'Content-Type': 'application/json' },
 ...
 case 2: return 'bg-gradient-to-r from-gray-300 to-gray-500 text-[#050505] border-gray-400 shadow-lg';
 case 3: return 'bg-gradient-to-r from-orange-400 to-orange-600 text-white border-orange-300 shadow-lg';
 default: return 'btn-chrome !rounded-full text-[#050505] border-[rgba(192,192,192,0.35)] shadow-md';
 }
 };
 ...
 break;
 case 'silver_plus':
 rankingColor = 'bg-gradient-to-r from-slate-300 to-slate-500 text-[#050505]';
 break;
 case 'silver':
 rankingColor = 'bg-gradient-to-r from-gray-400 to-gray-600 text-[#050505]';
 break;
 case 'bronze':
 default:
 ...
 <div className={`w-16 h-16 border-4 ${theme === 'dark' ? 'border-[rgba(192,192,192,0.22)] border-t-[var(--chrome-mid)]' : 'border-[rgba(192,192,192,0.22)] border-t-[var(--chrome-mid)]'} rounded-full animate-spin mx-auto mb-4`}></div>
 ...
 </header>  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"> {/* Stats Overview */}
 ...
 <div className="text-center">
 <div className={`text-2xl font-bold ${theme === 'dark' ? 'text-[var(--chrome-mid)]' : 'text-[var(--chrome-mid)]'}`}> {filteredRankings.length}
 </div>
 ...
 <div className={`${themeClasses.metricCardBg} ${themeClasses.cardRadius} p-6 ${themeClasses.cardShadow} border ${themeClasses.metricCardBorder} hover:shadow-2xl transition-all duration-300 transform hover:scale-105`}>
 <div className="text-center">
 <div className={`text-2xl font-bold ${theme === 'dark' ? 'text-[var(--chrome-mid)]' : 'text-green-600'}`}> {new Set(filteredRankings.map(r => r.studioName)).size}
 </div>
 ...
 <div className={`${themeClasses.metricCardBg} ${themeClasses.cardRadius} p-6 ${themeClasses.cardShadow} border ${themeClasses.metricCardBorder} hover:shadow-2xl transition-all duration-300 transform hover:scale-105`}>
 <div className="text-center">
 <div className={`text-2xl font-bold ${theme === 'dark' ? 'text-[var(--chrome-mid)]' : 'text-[var(--chrome-mid)]'}`}> {new Set(filteredRankings.map(r => r.ageCategory)).size}
 </div>
 ...
 <div className={`${themeClasses.metricCardBg} ${themeClasses.cardRadius} p-6 ${themeClasses.cardShadow} border ${themeClasses.metricCardBorder} hover:shadow-2xl transition-all duration-300 transform hover:scale-105`}>
 <div className="text-center">
 <div className={`text-2xl font-bold ${theme === 'dark' ? 'text-teal-400' : 'text-teal-600'}`}> {new Set(filteredRankings.map(r => r.itemStyle)).size}
 </div>
 <div className={`text-sm ${themeClasses.textSecondary} font-medium`}>Dance Styles</div>
 </div>
 </div>
 </div> {/* Nationals Breakdown */}
 <div className="glass-panel backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-[rgba(192,192,192,0.22)] mb-8">
 ...
 <div className="text-xs text-gray-400"> {((count / filteredRankings.length) * 100).toFixed(1)}% of total
 </div>
 </div>
 ...
 <span className="text-xs"> {index === 0 ? '' : index === 1 ? '' : ''}
 </span>
 <span className="text-xs text-gray-400 font-medium"> {index === 0 ? 'Most items' : index === 1 ? '2nd most' : '3rd most'}
 </span>
 </div> )}
 </div> ));
 })()}
 </div>  </div> {/* Enhanced Filters with View Mode Tabs */}
 <div className="glass-panel backdrop-blur-lg rounded-2xl shadow-xl p-8 mb-8 border border-[rgba(192,192,192,0.22)]"> {/* View Mode Tabs */}
 <div className="flex flex-wrap gap-2 mb-6">
 <button
 onClick={() => setViewMode('all')}
 className={`px-4 py-2 rounded-xl font-medium transition-all duration-200 ${
 viewMode === 'all'
 ? 'btn-chrome !rounded-full text-white shadow-lg'
 ...
 className="w-full px-4 py-3 border-2 border-gray-600 rounded-xl focus:ring-2 focus:ring-[rgba(192,192,192,0.45)] focus:border-[rgba(192,192,192,0.5)] transition-all duration-200 font-medium bg-gray-700 text-white" >
 ...
 className="w-full px-4 py-3 border-2 border-gray-600 rounded-xl focus:ring-2 focus:ring-[rgba(192,192,192,0.45)] focus:border-[rgba(192,192,192,0.5)] transition-all duration-200 font-medium bg-gray-700 text-white" >
 <option value="">All Ages</option> {['4 & Under', '6 & Under', '7-9', '10-12', '13-14', '15-17', '18-24', '25-39', '40+', '60+'].map(age => (
 ...
 className="w-full px-4 py-3 border-2 border-gray-600 rounded-xl focus:ring-2 focus:ring-[rgba(192,192,192,0.45)] focus:border-[rgba(192,192,192,0.5)] transition-all duration-200 font-medium bg-gray-700 text-white" >
 <option value="">All Types</option> {['Solo', 'Duet', 'Trio', 'Group'].map(type => (
 ...
 className="w-full px-4 py-3 border-2 border-gray-600 rounded-xl focus:ring-2 focus:ring-[rgba(192,192,192,0.45)] focus:border-[rgba(192,192,192,0.5)] transition-all duration-200 font-medium bg-gray-700 text-white" >
 <option value="">All Styles</option> {Array.from(new Set(rankings.map(r => r.itemStyle).filter(s => s))).sort().map(style => (
 ...
 className="w-full px-4 py-3 btn-chrome !rounded-full text-white rounded-xl disabled:opacity-50 transition-all duration-200 font-semibold shadow-md" >
 ...
 <div className="glass-panel backdrop-blur-lg rounded-2xl shadow-xl overflow-hidden border border-[rgba(192,192,192,0.22)]">
 <div className="chrome-bar px-8 py-6">
 <div className="flex items-center justify-between gap-4">
 <div>
 <h2 className="font-display text-2xl chrome-bar-title leading-none">Rankings</h2>
 <p className="chrome-bar-muted mt-1 text-sm uppercase tracking-wide">Performance results and awards - Filter by event to view specific competition results</p>
 </div>
 <div className="text-right shrink-0">
 <div className="text-sm chrome-bar-muted uppercase tracking-wide">Total Results</div>
 <div className="text-2xl font-bold text-[#050505]">{filteredRankings.length}</div>
 </div>
 </div>
 </div>  <div className="p-8"> {filteredRankings.length === 0 ? (
 <div className="text-center py-12 glass-panel rounded-2xl shadow-lg">
 <div className="text-6xl mb-4"></div>
 ...
 <th className="text-left py-4 px-6 font-bold text-white">Rank</th> {isRegionalEventSelected && (
 ...
 <tbody> {filteredRankings.map((ranking, index) => {
 ...
 return (
 <tr key={ranking.performanceId} className="border-b border-[rgba(192,192,192,0.15)] hover:bg-black/40 transition-colors">
 <td className="py-4 px-6">
 <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-bold border ${getRankBadgeColor(displayRank)}`}> {getRankIcon(displayRank)}
 </div>
 </td> {isRegionalEventSelected && (
 <td className="py-4 px-6">
 <label className="inline-flex items-center gap-2 text-sm text-white font-medium">
 <input
 type="checkbox" checked={!!ranking.announced}
 onChange={(e) => toggleAnnounced(ranking.performanceId, e.target.checked)}
 className="h-4 w-4 rounded border-gray-500 text-emerald-500 focus:ring-emerald-400 bg-gray-700" /> Announced
 </label>
 </td> )}
 <td className="py-4 px-6"> {ranking.itemNumber ? (
 <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold btn-chrome text-[#050505] border border-[rgba(192,192,192,0.35)] shadow-md"> Item {ranking.itemNumber}
 </div> ) : (
 ...
 ? 'btn-chrome !rounded-full text-[#050505] border-[rgba(192,192,192,0.35)]'
 : 'bg-gradient-to-r from-orange-500 to-red-600 text-white border-orange-300'
 }`}> {(ranking.entryType || 'live') === 'virtual' ? ' Virtual' : ' Unassigned'}
 </div> )}
 </td>
 <td className="py-4 px-6">
 <div className="font-semibold text-white">{ranking.title}</div>
 <div className="flex items-center gap-2 mt-1">
 <span className="text-sm text-gray-400">{ranking.itemStyle}</span> {ranking.mastery && (
 <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-emerald-700 text-emerald-200"> {ranking.mastery}
 </span> )}
 </div>
 </td>
 <td className="py-4 px-6">
 <div className="font-medium text-white">{ranking.contestantName}</div> {ranking.studioName && (
 <div className="text-xs text-gray-400 mt-1">{ranking.studioName}</div> )}
 </td>
 <td className="py-4 px-6">
 <div className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-blue-700 text-blue-200 border border-blue-500"> {ranking.performanceType}
 </div>
 </td>
 <td className="py-4 px-6">
 <div className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-indigo-700 text-indigo-200 border border-indigo-500"> {ranking.ageCategory}
 </div>
 </td>
 <td className="py-4 px-6">
 ...
 <span className="mr-1">{medalEmoji}</span> {rankingLevel}
 </span>
 </td>
 </tr> );
 })}
