...
export default function AnnouncerPortalPage() {
 ...
 try {
 const response = await fetch('/api/auth/announcer', {
 method: 'POST',
 ...
 if (result.success) {
 localStorage.setItem('announcerSession', JSON.stringify(result.user));
 router.push('/announcer-dashboard');
 } else {
 ...
 return (
 <AuthPortalLayout title="Announcer Portal" subtitle="Avalon Competition Management">
 <div className="p-8">
 <p className="text-[var(--muted-foreground)] text-sm text-center mb-6"> Announce performances and manage live event flow
 ...
 {isLoading ? 'Signing in...' : 'Sign in as Announcer'}
 </button>  <div className="mt-6">
 <ForgotPasswordLink userType="announcer" />
 </div>
