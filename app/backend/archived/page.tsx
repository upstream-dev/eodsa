...
export default function BackendArchivedPage() {
 ...
 <h1 className="text-3xl font-bold chrome-text"> Archived Events</h1>
 <p className="text-gray-300 text-sm mt-1"> Hidden from live dashboards. Scores, certificates, and entries are kept.
 </p>
 </div>
 <button
 type="button" onClick={loadArchived}
 ...
 <div className="glass-panel rounded-xl border border-emerald-500/30 p-5">
 <div className="flex items-center justify-between">
 <div>
 <p className="text-gray-400 text-sm">Media kept</p>
 <p className="text-3xl font-bold text-[var(--chrome-mid)]"> {loading ? '—' : Math.max(0, events.length - purgedCount)}
 </p>
 </div>
 <div className="text-4xl"></div>
 </div>
 </div>
 <div className="glass-panel rounded-xl border border-rose-500/30 p-5">
 <div className="flex items-center justify-between">
 <div>
 ...
 <p className="text-gray-400 text-sm mt-1"> Restore to bring an event back to Admin  Events, or delete media to free storage.
 </p>
 </div> {loading ? (
 ...
 <p className="text-gray-500 text-sm mt-2"> On Admin  Events, use Archive when an event is completed.
 </p>
 <Link
 href="/admin" className="inline-flex mt-5 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors" >
 Go to Admin Events
 </Link>
 </div> ) : (
 <div className="divide-y divide-purple-500/20"> {events.map((event) => (
 ...
 className="text-lg font-bold text-white hover:text-[var(--chrome-light)] transition-colors" >
 {event.name}
 </Link>
 <span className="px-3 py-1 rounded-full text-xs font-semibold bg-[rgba(192,192,192,0.08)] text-[var(--chrome-light)] border border-[rgba(192,192,192,0.22)]"> ARCHIVED
 </span> {event.mediaPurgedAt && (
 <span className="px-3 py-1 rounded-full text-xs font-semibold bg-rose-500/20 text-rose-300 border border-rose-500/30"> MEDIA PURGED
 </span> )}
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-1 text-sm">
 <p className="text-gray-300"> Date:{' '}
 <span className="text-white font-medium"> {event.eventDate ? new Date(event.eventDate).toLocaleDateString() : '—'}
 </span>
 </p>
 <p className="text-gray-300"> Venue: <span className="text-white font-medium">{event.venue || '—'}</span>
 </p>
 <p className="text-gray-300"> Archived:{' '}
 <span className="text-white font-medium"> {event.archivedAt
 ? new Date(event.archivedAt).toLocaleDateString()
 : '—'}
 </span>
 </p> {event.archivedBy && (
 <p className="text-gray-300"> By: <span className="text-white font-medium">{event.archivedBy}</span>
 </p> )}
 </div> {!event.mediaPurgedAt && (
 <p className="text-xs text-gray-500"> Delete music &amp; video frees Cloudinary storage. Scores and certificates stay.
 </p> )}
 ...
 <button
 type="button" onClick={() => restore(event)}
 ...
 <span className="px-4 py-2.5 text-sm text-gray-500 self-center"> Media already deleted
 </span> )}
 </div>
 </div>
 </div> ))}
 ...
 <div className="max-w-md w-full rounded-xl border border-[rgba(192,192,192,0.35)]/40 bg-gray-800 p-6 shadow-2xl">
 <h3 className="text-lg font-bold text-white mb-2">Delete music &amp; video files?</h3>
 <p className="text-sm text-gray-300 mb-3"> Permanently removes uploaded music and video for{' '}
 ...
 <li> Scores, certificates, entries, and payments are{' '}
 <strong className="text-white">not</strong> deleted
 </li>
 <li>Sound desk will no longer have those files</li>
 <li>This cannot be undone</li>
 </ul>
 <p className="text-sm text-gray-300 mb-2"> Type <span className="font-mono font-bold text-white">PURGE</span> to confirm.
 </p>
 <input
 type="text" value={purgeText}
 onChange={(e) => setPurgeText(e.target.value)}
 placeholder="Type PURGE" className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:ring-[3px] focus:ring-[rgba(192,192,192,0.12)] mb-4" autoFocus
 />
 ...
 <button
 type="button" onClick={confirmPurge}
